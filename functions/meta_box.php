...
function thd_meta_box_callback($fields, $page) {
	...
	foreach ($fields as $field) {
		...
		if (isset($field['label']) && $field['label'])	$label 		= $field['label'];
		if (isset($field['desc']) && $field['desc']) 	$desc 		= '<span class="description">'.$field['desc'].'</span>';
		if (isset($field['id']) && $field['id']) 		$id 		= $field['id'];
		if (isset($field['type']) && $field['type']) 	$type 		= $field['type'];
		if (isset($field['value']) && $field['value']) 	$value 		= $field['value'];
		if (isset($field['options']) && $field['options']) 	$options	= $field['options'];
		...
		echo '<tr>
				...
				switch($field['type']) {
					// text
					case 'text':
						echo '<input type="text" name="'.$id.'" id="'.$id.'" value="'.$meta.'" size="20" />
								<br />'.$desc;
					break;
					// textarea
					case 'textarea':
    					...
    					if ($meta) {  
        					foreach($meta as $row) {  
            					echo '<li><span class="sort hndle">|||</span> 
                        			...
                        			<textarea type="text" name="'.$field['id'].'['.$i.']" id="'.$field['id'].'" style="width: 90%;" rows="4" />'. htmlentities($row).'</textarea>
									<a class="repeatable-remove button" href="#">-</a></li>';  
            					$i++;  
        					}  
    					} else {  
        					echo '<li><span class="sort hndle">|||</span> 
                    			<textarea type="text" name="'.$field['id'].'['.$i.']" id="'.$field['id'].'" style="width: 90%;" rows="4" /></textarea>
                    			<a class="repeatable-remove button" href="#">-</a></li>';
    					}  
    					echo '</ul> 
